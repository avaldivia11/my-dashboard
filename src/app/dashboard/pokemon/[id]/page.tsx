...
export async function generateStaticParams() {
  ...
  return static151Pokemons.map((idPokemon) => ({ id: idPokemon }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `#${id} - ${capitalizeWords(name)}`,
      description: `Página del pokémon ${capitalizeWords(name)}`
    }
        ...
        <div className='mt-2 mb-8 w-full'>
          ...
          <div className='flex flex-col justify-center items-center'>
            ...
            <div className='flex flex-wrap items-center'>
              <span className='text-xl mr-2'>Movements:</span>
              {pokemon.moves.map((move) => (
                <div
                  key={move.move.name}
                  className={`mr-2 my-1 px-2 rounded-2xl border-1 ${getPokemonTypeColor(
                    pokemon.types
                  )}`}
                >
